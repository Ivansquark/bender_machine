...
static void low_level_init(struct netif* netif) {
    ...
    netif->hwaddr[0] = Eth::pThis->MY_MAC[0];
    netif->hwaddr[1] = Eth::pThis->MY_MAC[1];
    netif->hwaddr[2] = Eth::pThis->MY_MAC[2];
    netif->hwaddr[3] = Eth::pThis->MY_MAC[3];
    netif->hwaddr[4] = Eth::pThis->MY_MAC[4];
    netif->hwaddr[5] = Eth::pThis->MY_MAC[5];
...
static struct pbuf* low_level_input(struct netif* netif) {
    ...
    if (Eth::ptrRxBuf == nullptr) { return NULL; }
    ...
    if (Eth::lenRxBuf != 0) {
        ...
        if (p != NULL) {
            ...
            for (q = p; q != NULL; q = q->next) {
                ...
                // once_size);
                memcpy(q->payload, (void*)(Eth::ptrRxBuf + read_index),
                       once_size);
